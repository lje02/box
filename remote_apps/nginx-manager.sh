...
EOF
        ...
        # 非标准 HTTP 端口：仅监听指定端口
        cat <<EOF
# 非标准 HTTP 端口强制跳转：${http_port} → HTTPS:${https_port}
server {
    listen ${http_port};
    server_name $domain;
    return 301 ${target_url};
...
EOF
        warn "非标准端口 ${http_port} 的 301 强转：浏览器须先访问 http://${domain}:${http_port}/ 才会触发跳转"
    fi
}
