...
install_warp_official() {
    ...
    if [[ "$status" == *"Registered"* ]]; then
        ...
        # 使用 --accept-tos 自动接受协议，保留 stderr 以便出现真实错误时输出
        if warp-cli registration new --accept-tos 2>/dev/null; then
            echo -e "${GREEN}✔ 注册命令已执行${PLAIN}"
        else
            echo -e "${RED}✘ 注册失败（warp-cli registration new 返回非 0）。请查看 warp-cli status 或 journalctl -u warp-svc 以获取更多信息。${PLAIN}"
        fi
    fi
    ...
    while true; do
        ...
        if [ $retry -ge $max_retry ]; then
            echo -e "${RED}✘ 自动连接超时（$((max_retry * 3))s），请后续执行 warp-cli status 检查${PLAIN}"
...
edit_node() {
    ...
    case $op in
        ...
        4)
            ...
            if [[ "$confirm" == "y" ]]; then
                # 核心优化：
                # 1. 寻找 route.rules 中 inbound 匹配当前 TAG 的项并删除（支持字符串和数组形式）
                # 2. 删除 inbounds 中对应的节点
                jq --arg tag "$TAG" '
                    (if .route.rules then 
    ...
    if [[ -f "tmp.json" ]]; then
        if save_and_restart; then
            ...
            [[ -z "$CONF" ]] && return # 如果是删除操作其实已经 return 了，这里做个保险

            local PORT=$(echo "$CONF" | jq -r .listen_port)
            local IP=$(get_ip)
            local SNI=$(echo "$CONF" | jq -r '.tls.server_name // ""')
            local HOST=$IP # 默认使用 IP 连通，防止自签证书 SNI 无法解析
            local NEW_LINK=""
